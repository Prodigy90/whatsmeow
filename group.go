...
func (cli *Client) GetGroupRequestParticipants(ctx context.Context, jid types.JID) ([]types.GroupParticipantRequest, error) {
	...
	for i, req := range requestParticipants {
		...
		// WhatsApp includes the requester's phone_number alongside their LID on
		// membership requests — capture it so join-request approvers resolve a PN
		// instead of "Hidden number" (tulir/whatsmeow#1163).
...
func (cli *Client) GetGroupInfoBatch(ctx context.Context, jids []types.JID) ([]*types.GroupInfo, error) {
	...
	for start := 0; start < len(jids); start += MaxGroupInfoBatchSize {
		...
		if !ok {
			...
			cli.Log.Warnf("Group info batch: no <groups> child in response for %d groups; response=%s", len(chunk), resp.String())
			continue
		}
